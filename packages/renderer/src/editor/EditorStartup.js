...
import editorTemplate from "./templates/editorTemplate.html.js";
...
class EditorStartup {
    ...
    async init() {
        ...
        $id("selLayerNames").addEventListener("change", (evt) => {
            ...
            const confirmStr = this.i18next
                .t("notification.QmoveElemsToLayer")
                .replace("%s", destLayer);
            /**
             * @param {boolean} ok
             * @returns {void}
        ...
        const addListenerMulti = (element, eventNames, listener) => {
            eventNames
                .split(" ")
                .forEach((eventName) => element.addEventListener(eventName, listener, false));
        };
        ...
        $id("se-svg-editor-dialog").addEventListener(
            "change",
            function (e) {
        ...
        $id("se-cmenu_canvas").addEventListener(
            "change",
            function (e) {
            ...
            function () {
                ...
                if (this.configObj.curConfig.snappingStep) {
                    $editDialog.setAttribute(
                        "gridsnappingstep",
                        this.configObj.curConfig.snappingStep
                    );
                }
    ...
    async extAndLocaleFunc() {
        ...
        try {
            ...
            // MG cf. https://github.com/rollup/plugins/tree/master/packages/dynamic-import-vars#limitations
            // await Promise.all(
