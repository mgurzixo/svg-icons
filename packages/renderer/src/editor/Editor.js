...
class Editor extends EditorStartup {
    ...
    zoomChanged(win, bbox, autoCenter) {
        ...
        const zInfo = this.svgCanvas.setBBoxZoom(
            bbox,
            parseFloat(getComputedStyle(this.workarea, null).width.replace("px", "")) - scrbar,
            parseFloat(getComputedStyle(this.workarea, null).height.replace("px", "")) - scrbar
        );
        if (!zInfo) {
            return;
        }
        ...
        if (context) {
            ...
            parentsUntil.forEach(function (parent) {
                if (parent.id) {
                    str += " > " + parent.id;
                    linkStr +=
                        parent !== context ? ` > <a href="#">${parent.id}</a>` : ` > ${parent.id}`;
                }
            });
    ...
    loadFromDataURI(str, { noAlert } = {}) {
        return this.ready(() => {
            ...
            return this.loadSvgString(base64 ? decode64(src) : decodeURIComponent(src), {
                noAlert,
            });
        });
    }
