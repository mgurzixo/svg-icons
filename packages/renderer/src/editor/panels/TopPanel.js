...
import topPanelHTML from "./TopPanel.html.js";
...
class TopPanel {
    ...
    promptImgURL({ cancelDeletes = false } = {}) {
        ...
        const url = prompt(this.editor.i18next.t("notification.enterNewImgURL"), curhref);
        if (url) {
    ...
    updateContextPanel() {
        ...
        if (elem) {
            ...
            if (panels[tagName]) {
                ...
                if (tagName === "text") {
                    ...
                    // text
                } else if (tagName === "image" && this.editor.svgCanvas.getMode() === "image") {
            ...
            menuItems.setAttribute(
                (tagName === "g" || !this.multiselected ? "dis" : "en") + "ablemenuitems",
                "#group"
            );
        ...
        $id("tool_undo").disabled = this.editor.svgCanvas.undoMgr.getUndoStackSize() === 0;
        $id("tool_redo").disabled = this.editor.svgCanvas.undoMgr.getRedoStackSize() === 0;
    ...
    makeHyperlink() {
        if (this.editor.selectedElement || this.multiselected) {
            const url = prompt(this.editor.i18next.t("notification.enterNewLinkURL"), "http://");
            if (url) {
    ...
    init() {
        ...
        [
            ...
            "path_node_x",
            "path_node_y",
        ].forEach((attrId) => $id(attrId).addEventListener("change", this.attrChanger.bind(this)));
    }
}
