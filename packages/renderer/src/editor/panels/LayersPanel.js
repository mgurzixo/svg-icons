...
import LayersPanelHtml from "./LayersPanel.html.js";
...
class LayersPanel {
     ...
     */
    cloneLayer() {
        const name = this.editor.svgCanvas.getCurrentDrawing().getCurrentLayerName() + " copy";

        const newName = prompt(this.editor.i18next.t("notification.enterUniqueLayerName"), name);
        if (!newName) {
            return;
        }
        if (this.editor.svgCanvas.getCurrentDrawing().hasLayer(newName)) {
    ...
    layerRename() {
        ...
        if (oldName === newName || this.editor.svgCanvas.getCurrentDrawing().hasLayer(newName)) {
        ...
        if (layerNameToHighlight) {
            curNames.forEach((curName) => {
                if (curName !== layerNameToHighlight) {
                    this.editor.svgCanvas.getCurrentDrawing().setLayerOpacity(curName, 0.5);
                }
            });
        } else {
            curNames.forEach((curName) => {
    ...
    populateLayers() {
        ...
        while (layer--) {
            ...
            layerVis.className = !drawing.getLayerVisibility(name)
                ? "layerinvis layervis"
                : "layervis";
